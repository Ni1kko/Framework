...
{
   //--- THIRST
    if ((time - _waterTime) > 900 && ((player getVariable ["lifeState",""]) isEqualTo "HEALTHY")) then {
    ...
    //--- HUNGER
    if ((time - _foodTime) > 1250 && ((player getVariable ["lifeState",""]) isEqualTo "HEALTHY")) then {
    ...
    //--- VIEW DISTANCE
    if (!(vehicle player isEqualTo _lastState) || {((player getVariable ["lifeState",""]) isNotEqualTo "HEALTHY")}) then {
    ...
    //--- WALKING 
    if ((player getVariable ["lifeState",""]) isNotEqualTo "HEALTHY") then {_walkDis = 0;} else {
