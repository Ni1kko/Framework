...
if(_adminlevel > 0) then {
	private _new = _paycheck * (parseNumber(format["0.%1",(9 min (1 max _adminlevel))]) + 8.75);
	...
	if(_donorlevel > 0) then {
		private _new = _paycheck * (parseNumber(format["0.%1",(9 min (1 max _donorlevel))]) + 4.20);
		private _diff = _new - _paycheck;
		_paycheck = _new;
...
life_var_paycheckThread = [_paycheck] spawn {
	...
	while {_paycheck > 0} do {
		...
		_time = diag_tickTime;
		["ADD","CASH",_paycheck] call MPClient_fnc_handleMoney;
		systemChat format["%1 You have received $%2 from your paycheck.",profileName, _paycheck];
	};
	true
};
