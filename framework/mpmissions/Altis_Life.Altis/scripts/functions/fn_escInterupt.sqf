...
with missionNamespace do
{
    ...
    { 
        ...
        //Enable Contols  
        if (call (missionNamespace getVariable ["MPClient_fnc_isDormant",{false}])) then
        {
            //Respawn Btn
            ...
            waitUntil {
                ...
                if(((_abortReady AND _syncReady) OR {isNull _display OR {not(call (missionNamespace getVariable ["MPClient_fnc_isDormant",{false}]))}}))exitWith{
                    true
                };
            ...
            waitUntil {
                ...
                if(not(call (missionNamespace getVariable ["MPClient_fnc_isDormant",{false}])) OR {isNull (uiNamespace getVariable ["RscDisplayMPInterrupt",displayNull])})exitWith{
                    true
                };
                false
            };
            ...
            waitUntil {
                ...
                if((call (missionNamespace getVariable ["MPClient_fnc_isDormant",{false}])) OR {isNull (uiNamespace getVariable ["RscDisplayMPInterrupt",displayNull])})exitWith{
                    true
                };
