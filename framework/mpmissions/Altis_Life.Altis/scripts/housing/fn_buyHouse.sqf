...
if (_action) then {
    if (life_var_bank < _cost) exitWith {hint format [localize "STR_House_NotEnough"]};
    ...
    if (count extdb_var_database_headless_clients > 0) then {
        ...
        [_uid,_house] remoteExec ["MPServer_fnc_addHouse",RSERV];
    };

    if (LIFE_SETTINGS(getNumber,"player_advancedLog") isEqualTo 1) then {
        if (LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging") isEqualTo 1) then {
            advanced_log = format [localize "STR_DL_AL_boughtHouse_BEF",[_cost] call MPClient_fnc_numberText,[life_var_bank] call MPClient_fnc_numberText,[life_var_cash] call MPClient_fnc_numberText];
        } else {
            advanced_log = format [localize "STR_DL_AL_boughtHouse",profileName,(getPlayerUID player),[_cost] call MPClient_fnc_numberText,[life_var_bank] call MPClient_fnc_numberText,[life_var_cash] call MPClient_fnc_numberText];
        };
