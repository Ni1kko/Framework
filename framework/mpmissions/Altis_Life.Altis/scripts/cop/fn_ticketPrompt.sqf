...
CONTROL(2600,2601) ctrlSetStructuredText parseText format [
    "<t align='center'><t size='.8px'>" +(localize "STR_Cop_Ticket_GUI_Given"),
    _cop getVariable ["realname",name _cop],
    _val
];

[] spawn {
    scriptName 'MPClient_fnc_ticketPaid';
    disableSerialization;
    waitUntil {life_ticket_paid || (isNull (findDisplay 2600))};
    if (isNull (findDisplay 2600) && !life_ticket_paid) then {
        [0,"STR_Cop_Ticket_Refuse",true,[profileName]] remoteExecCall ["MPClient_fnc_broadcast",west];
        [1,"STR_Cop_Ticket_Refuse",true,[profileName]] remoteExecCall ["MPClient_fnc_broadcast",life_ticket_cop];
    };
};