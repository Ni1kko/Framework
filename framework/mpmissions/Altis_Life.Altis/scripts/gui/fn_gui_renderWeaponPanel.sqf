...
_display = uiNamespace getVariable "RscTitleHUD";
...
if (_weaponPanelType isEqualTo 0) then {
	...
	if !(_currentMagazineClassName isEqualTo life_var_hud_lastrendered_muzzle) then {
		...
		life_var_hud_lastrendered_muzzle = _currentMagazineClassName;
	};
};